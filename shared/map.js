/* ============================================================
   shared/map.js — Classic Risk map + the 8 trivia categories.
   UMD: loadable via require() (server, tests) and <script> (browser).
   Territory categories are NOT stored here — they are assigned
   randomly per game during setup and live in the game state.
   ============================================================ */
(function (root, factory) {
  if (typeof module === "object" && module.exports) module.exports = factory();
  else root.TriviaRiskMap = factory();
})(typeof self !== "undefined" ? self : this, function () {
  // The 8 trivia categories — exact strings, order is the canonical index.
  const CATEGORIES = [
    "World & Geography",
    "History & Dates",
    "Science & Nature",
  ...
  // A color/emoji badge per category for the UI.
  const CATEGORY_BADGES = {
    "World & Geography": { color: "#3f9b6d", icon: "🌍" },
    "History & Dates": { color: "#b58235", icon: "📜" },
    "Science & Nature": { color: "#3f7fb5", icon: "🔬" },
    "Sports": { color: "#c4543a", icon: "🏅" },
    "Entertainment & Pop Culture": { color: "#a85fb0", icon: "🎬" },
    "Business & Money": { color: "#2f8f8f", icon: "💰" },
    "Food & Everyday Life": { color: "#cf8a3e", icon: "🍔" },
    "Wild Stats": { color: "#8a6fc0", icon: "📊" },
  };

  // Continents and (optional) army bonuses — preserved from classic Risk.
  const CONTINENTS = {
  ...
  // Player palette (player setup may override per-player color).
  const PLAYER_COLORS = ["#e54b4b", "#4b7fe5", "#46b06a", "#e5c84b"];
  const PLAYER_NAMES_DEFAULT = ["Red", "Blue", "Green", "Gold"];
